...
const Navbar = () => {
    ...
    return (
        <div className='flex items-center justify-between text-sm py-4 mb-5 border-b border-b-gray-400 cursor-pointer'>
            <img onClick={() => navigate('/')} className='w-24 h-22 cursor-pointer' src={assets.logo} alt="Logo" />
            ...
            <div className='flex items-center gap-4'>
                ...
                <div className={`${showMenu ? 'fixed w-full' : 'h-0 w-0'} md:hidden right-0 top-0 bottom-0 z-20 overflow-hidden bg-white transition-all`}>
                    <div className='flex items-center justify-between px-5 py-6'>
                        ...
                        <img className='w-7' onClick={() => setShowMenu(false)} src={assets.cross_icon} alt="exit" />
                    </div>
                    <ul className='flex flex-col items-center gap-2 mt-5 px-5 text-lg font-medium'>
                        <NavLink onClick={() => setShowMenu(false)} to='/'><p className='px-4 py-2 rounded inline-block'>HOME</p></NavLink>
                        <NavLink onClick={() => setShowMenu(false)} to='/coaches'><p className='px-4 py-2 rounded inline-block'>COACHES</p></NavLink>
